# Web Site Design – Semester Coursework Portfolio

---

## Final Exam Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

---

## Table of Contents
- [Homework 01: Personal Profile Page](#homework-01-personal-profile-page)
- [Homework 02: Personal Information Form](#homework-02-personal-information-form)
- [Homework 03: JavaScript Hello World](#homework-03-javascript-hello-world)
- [Homework 04: JavaScript Fundamentals Exercises](#homework-04-javascript-fundamentals-exercises)
- [Homework 05: Progressive Web Development (HTML → Full-Stack)](#homework-05-progressive-web-development-html--full-stack)
- [Homework 06: JavaScript Advanced Concepts](#homework-06-javascript-advanced-concepts)
- [Homework 07: JavaScript Practical Applications](#homework-07-javascript-practical-applications)
- [Midterm Exam Project: Student Management System](#midterm-exam-project-student-management-system)

---

# Homework 01: Personal Profile Page

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A personal profile card page built with HTML and CSS. Displays the student's name, student ID, and department in a centered card layout with a blue avatar circle and clean typography.

## Files Included
- aboutme.html

## How to Run
Open `aboutme.html` in any modern web browser.

---

# Homework 02: Personal Information Form

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A personal data entry form built with HTML and CSS. Includes fields for name, gender, date of birth, email, phone, occupation, and address with form validation and styled UI components.

## Files Included
- form.html

## How to Run
Open `form.html` in any modern web browser.

---

# Homework 03: JavaScript Hello World

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A simple JavaScript script that prints 'Hello, World!' to the console. Serves as an introduction to JavaScript programming and Node.js execution.

## Files Included
- hello.js

## How to Run
Run `node hello.js` in your terminal.

---

# Homework 04: JavaScript Fundamentals Exercises

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A set of 10 JavaScript exercises covering core programming concepts including conditional logic (if-else), loops (while), arrays, objects, JSON parsing, and functions.

## Files Included
- 01_Basic_Grade_Classifier.js
- 02_The_Accumulator.js
- 03_Find_the_Smallest_Number.js
- 04_User_Profile_Management.js
- 05_Filtering_Students.js
- 06_JSON_Data_Parsing.js
- 07_Shopping_Cart_Total.js
- 08_Search_for_a_Fruit.js
- 09_Nested_Data_Access.js
- 10_Factorial_Calculator.js

## How to Run
Run each `.js` file with `node <filename>.js` in your terminal.

---

# Homework 05: Progressive Web Development (HTML → Full-Stack)

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A progressive multi-version web project that evolves through five stages: a basic HTML page (v1-basic), CSS-styled version (v2-css), JavaScript-enhanced interactivity (v3-javascript), a Node.js/Express backend server (v4-nodejs), and a full-stack application with SQLite database integration (v5-fullstack).

## Files Included
- v1-basic/index.html
- v2-css/index.html
- v3-javascript/index.html
- v4-nodejs/package-lock.json
- v4-nodejs/package.json
- v4-nodejs/server.js
- v5-fullstack/blog.db
- v5-fullstack/database.js
- v5-fullstack/package-lock.json
- v5-fullstack/package.json
- v5-fullstack/server.js

## How to Run
Navigate to `v4-nodejs`, run `node server.js`, then open http://localhost:3000.

---

# Homework 06: JavaScript Advanced Concepts

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A collection of JavaScript exercises exploring advanced topics such as callbacks, Immediately Invoked Function Expressions (IIFE), arrow functions, higher-order functions, pass-by-reference behavior, and asynchronous delayed execution with setTimeout.

## Files Included
- 01.Callback.js
- 02.IIFE.js
- 03.ArrowFunction.js
- 04.DestructiveModify.js
- 05.HigherOrderFunction.js
- 06.CallbackFilter.js
- 07.ObjectArrow.js
- 08.PassByReference.js
- 09.DelayedCallback.js
- 10.CalculateTotal.js

## How to Run
Run each `.js` file with `node <filename>.js` in your terminal.

---

# Homework 07: JavaScript Practical Applications

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A collection of JavaScript exercises focusing on practical programming patterns including property access, destructuring assignment, array forEach iteration, URL query parameter parsing, callbacks, JSON parsing, mock database queries, template logic, string sorting, and error handling.

## Files Included
- 01.property_access.js
- 02.destructuring.js
- 03.array_foreach.js
- 04.url_params.js
- 05.callback.js
- 06.json_parse.js
- 07.db_query.js
- 08.template_logic.js
- 09.sort_substring.js
- 10.error_callback.js

## How to Run
Run each `.js` file with `node <filename>.js` in your terminal.

---

# Midterm Exam Project: Student Management System

## AI and Originality Declaration
* **Project Generation:** This project/file was generated using OpenCode.
* **Automation:** OpenCode (AI-powered Python Code Interpreter) was used to automate the structure, file scanning, and generation of this README documentation.

## Assignment Description
A full-stack web application for managing school operations including student records, courses, and enrollments. Built with a Node.js/Express backend, SQLite database (sql.js), and a vanilla HTML/CSS/JavaScript frontend. Features full CRUD operations, a real-time dashboard, dynamic sorting, sequential ID re-indexing, and a numeric grade system (0-100).

## Files Included
- package-lock.json
- package.json
- public/app.js
- public/index.html
- public/styles.css
- school.db
- server.js

## How to Run
Run `npm install` then `npm start` (or `node server.js`). Open http://localhost:3000.

---